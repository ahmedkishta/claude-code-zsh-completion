...
_claude_mcp_servers() {
  ...
  # Čtení přímo z konfiguračních souborů místo spouštění 'claude mcp list'
  for config_file in ~/.claude/mcp.json ~/.claude.json ~/.config/claude/mcp.json; do
    [[ -f "$config_file" ]] || continue

    # Extrakce názvů serverů z JSON (sekce mcpServers)
  ...
  # Záložní varianta: claude mcp list, pokud parsování konfigurace selže
...
_claude_installed_plugins() {
  ...
  # Kontrola adresářů pluginů přímo
  ...
  _describe 'nainstalované pluginy' plugins
}
...
_claude_sessions() {
  ...
  # Kontrola adresáře relací
  for session_dir in ~/.claude/sessions ~/.config/claude/sessions; do
    [[ -d "$session_dir" ]] || continue

    # Extrakce UUID přímo z názvů souborů
    sessions+=(${session_dir}/*~*.zwc(N:t:r))
  done

  # Filtrování pouze platných UUID
  sessions=(${(M)sessions:#[0-9a-f](#c8)-[0-9a-f](#c4)-[0-9a-f](#c4)-[0-9a-f](#c4)-[0-9a-f](#c12)})

  _describe 'identifikátory relací' sessions
}
...
_claude() {
  ...
  main_commands=(
    ...
    'migrate-installer:Migrace z globální npm instalace na lokální instalaci'
